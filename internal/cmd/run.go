...
curl https://charm.land | star run "Summarize this website"

# Read from a file
star run "What is this code doing?" <<< prrr.go

# Redirect output to a file
star run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
star run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
star run --verbose "Generate a README for this project"

# Continue a previous session
star run --session {session-id} "Follow up on your last response"

# Continue the most recent session
star run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'star' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `star run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `star run` exited
...
// prefix returned by star session list.
